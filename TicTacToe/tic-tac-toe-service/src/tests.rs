...
mod auto_player_tests {
    ...
    fn test_get_empty_locations() {
        ...
        let empty_locations = AutomaticPlayer::<TicTacToeGame>::determine_empty_locations(&board_state.new_game_state.game_board).unwrap_or_default();
...
mod game_board_tests {
    ...
    fn test_winning_moves() {
        ...
        assert_eq!(board_state.winning_player.unwrap_or_default().player_id, player_one.player_id);
        ...
        assert_eq!(board_state.new_game_state.play_status, PlayStatus::EndedInWin);
        assert_eq!(board_state.winning_player.unwrap_or_default().player_id, player_two.player_id);
    }
}
...
mod game_play_tests {
    ...
    fn test_get_current_board_state() {
        ...
        let mut game = TicTacToeGame::new(GameMode::TwoPlayers, Uuid::new_v4().to_string().as_str()).unwrap_or_default();
    ...
    fn test_take_turn() {
        ...
        let first_player_game_piece = game.current_player.clone().unwrap_or_default().game_piece;

        // Let Player One take their turn
        let turn_info = GameTurnParams {
            destination: first_destination.clone(),
            player_id: game.current_player.clone().unwrap_or_default().player_id,
            session_id: "".to_string(),
        };
        match game.take_turn(&turn_info) {
            Ok(_) => {}
        ...
        let second_player_game_piece = game.current_player.clone().unwrap_or_default().game_piece;
